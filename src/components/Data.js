...
export async function isSavedPost(userid,postid)
{
    const savedposts = collection(db,"savedposts");
    const q = query(savedposts,where("postid","==",postid),where("userid","==",userid));
    const docs = await getDocs(q);
    return docs.size!=0;
}
...
{
    const savedposts=collection(db,"savedposts");
    const q= query(savedposts,where("userid","==",userid));
    const docs=await getDocs(q);
    const collectedPosts=[];
    await docs.forEach(async (docx)=>
    {
        const {postid} = docx.data();
...
export async function addSavedPost(userid,postid)
{
    const savedposts = collection(db,"savedposts");
    addDoc(savedposts,{userid:userid,postid:postid});
}

export async function deleteSavedPost(userid,postid)
{
    const savedposts = collection(db,"savedposts");
    const q = query(savedposts,where("postid","==",postid),where("userid","==",userid));
    const docs = await getDocs(q);
    docs.forEach(async (docx)=>{deleteDoc(doc(db,"savedposts",docx.id))});
}
